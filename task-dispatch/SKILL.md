...
- **子代理并行上限：5个**（压测验证6个会gateway timeout，5个稳定）
- 三模型搜索场景同时3个
- 多任务场景：分批执行，前一批完成（或至少1个完成）再启动下一批

## 超时与长任务处理

- **runTimeoutSeconds 要给够**：复杂调研/搜索任务至少300s，简单任务60s
- **子代理超时 ≠ 任务失败**：子代理可能已完成工作（写了文件）但来不及返回结果。超时后先检查产出文件是否已存在
- **长任务策略**：子代理可以长时间做任务，主对话定时检查状态（sessions_list/sessions_history）并向用户汇报进度
- **检查方法**：
  1. `sessions_list` 查看子代理是否还在运行
  2. `sessions_history` 查看最新进展
  3. 检查子代理应写入的目标文件是否已生成
- **汇报节奏**：每1-2分钟向用户简要汇报一次状态，不要让用户干等无反馈

## 进度汇报（必做）

**多步骤/多子代理任务必须主动向用户汇报进度。** 不要让用户猜任务到哪了。

### 何时汇报
- **每个子代理完成时**：简短一句话告知结果（成功/失败/需返工）
- **多路并行时**：N路中第1路完成时汇报"1/3完成"，全部完成时汇报总结
- **阶段切换时**：如"生成阶段完成，进入交叉质询阶段"
- **发现问题时**：如"验收发现结构不合规，需要返工"
- **长时间无进展时**：超过3分钟没有子代理返回，主动check并汇报等待状态

### 汇报原则
- **简短有料**：一两句话说清进度+关键发现，不要长篇大论
- **问题优先**：有问题的先说，没问题的一笔带过
- **带数字**：3/5完成、2份合格1份需返工、预计还需X分钟
- **不要等全部完成才汇报**：中间状态也要说，用户需要知道事情在推进
- **失败要立刻说**：子代理失败/结构不合规/质量不达标，第一时间通知用户并说明下一步

### 反面教材
- ❌ 5个子代理静默跑了15分钟，最后一次性甩出5份结果
- ❌ 发现问题但不说，默默返工，用户不知道发生了什么
- ❌ 只说"在处理中"没有具体信息
- ✅ "数学卷A完成(19题/150分✅)，B在跑，C排队中"
- ✅ "验收发现3份数学卷都有硬伤，物理A可用。数学需要重做，我先启动新一轮"
